...
or set BBR_CHIAVDF_DIR to a chiavdf checkout.",
    ...
    if target_os == "windows" {
        ...
        if force_windows_fallback {
            ...
            build_windows_fast_path(&manifest_dir, &chiavdf_dir, &chiavdf_src);
        }
        return;
    }
...
fn build_windows_fast_path(manifest_dir: &PathBuf, chiavdf_dir: &PathBuf, chiavdf_src: &PathBuf) {
    ...
    let asm_objects = match build_windows_asm_objects(&clang_cl, chiavdf_src, &mpir_dir, &out_dir) {
        Ok(v) => v,
        Err(msg) => {
            println!(
                "cargo:warning=Windows fast-path asm generation failed ({msg}); falling back to portable implementation. You can also set BBR_FORCE_WINDOWS_FALLBACK=1."
            );
            build_windows_fallback(manifest_dir, chiavdf_dir, chiavdf_src);
            return;
        }
