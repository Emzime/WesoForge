...
use bbr_client_chiavdf_fast::ChiavdfBatchJob;
        ...
        let compute =
            tokio::task::spawn_blocking(move || -> anyhow::Result<Vec<(Vec<u8>, bool)>> {
                ...
                let results = if progress_steps == 0 {
                    let (_backend, results) = bbr_client_compute::prove_batch(
                        &challenge,
                        &x,
                        DISCRIMINANT_BITS,
                        &batch_jobs,
                    )
                    .context("compute prove_batch")?;
                    results
                } else {
                    let progress_for_cb = progress_clone.clone();
                    let (_backend, results) = bbr_client_compute::prove_batch_with_progress(
                        &challenge,
                        &x,
                        DISCRIMINANT_BITS,
                        &batch_jobs,
                    ...
                    .context("compute prove_batch_with_progress")?;
                    results
                };
    ...
    loop {
        ...
        let compute = tokio::task::spawn_blocking(move || -> anyhow::Result<(Vec<u8>, bool)> {
            ...
            let out = if progress_steps == 0 {
                let (_backend, out) = bbr_client_compute::prove_single(
                    &challenge,
                    &x,
                    &output,
                    DISCRIMINANT_BITS,
                ...
                .context("compute prove_single")?;
                out
            } else {
                let progress_for_cb = progress_clone.clone();
                let (_backend, out) = bbr_client_compute::prove_single_with_progress(
                    &challenge,
                    &x,
                    &output,
                    DISCRIMINANT_BITS,
                ...
                .context("compute prove_single_with_progress")?;
                out
            };
