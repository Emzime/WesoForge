...
pub fn add1_execute(device_index: usize, input: &[u32]) -> anyhow::Result<Vec<u32>> {
    ...
    // Safety: the kernel writes every output element before we copy to host.
    let out_dev: DeviceBuffer<u32> = unsafe {
...
/// Execute the VDF proof batch kernel on the GPU.
///
/// Input layout:
/// - `challenges_words` must contain `jobs * 8` u32 words (32 bytes per job).
///
...
/// - Replace `src/vdf.ptx` with your actual kernel PTX without changing any Rust plumbing.
