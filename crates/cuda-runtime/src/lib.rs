...
pub fn add1_smoketest(device_index: usize, n: usize) -> anyhow::Result<()> {
    ...
    // Compile/load PTX module.
    let ptx = ADD1_PTX;
    let module = unsafe { Module::from_ptx(ptx, &[]) }.context("failed to load PTX module")?;
...
///
/// This is a building block for wiring a real "pack -> H2D -> kernel -> D2H -> unpack" pipeline.
/// The kernel computes: `out[i] = input[i] + 1`.
pub fn add1_execute(device_index: usize, input: &[u32]) -> anyhow::Result<Vec<u32>> {
    ...
    let n_u32: u32 = u32::try_from(n).context("input too large for u32")?;

    // If input is empty, still run a 1-element launch with a dummy value to keep the path uniform.
    ...
    // Trim the dummy element if the original input was empty.
    if input.is_empty() {
        return Ok(Vec::new());
    }
